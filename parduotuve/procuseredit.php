...
//{ header("Location: logout.php");exit;}
  ...
  list(,,,,$upass)=checkdb($mail); //paimam slaptazodzio maisa is DB
	   if (!$upass)
	   { echo " DB klaida nuskaitant slaptazodi vartotojui ".$mail;
		               exit;}

  if (checkpass($pass,$upass))  // senas slaptazodis ivestas geras  
    {$ar_pasnn=checkpass($passn,substr(hash('sha256',$passn),5,32)); // ar geras naujas
	 ...
	 if ( $ar_pasnn && $ar_mail)  // lauku reiksmes geros
      { if ($pass != $passn || $mail != $_SESSION['umail'])   // vartotojas kazka keicia
	            {$upass=substr(hash('sha256',$passn),5,32);
                 $db=mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
                 $sql = "UPDATE ". TBL_USERS." SET slaptazodis='$upass' WHERE  el_pastas='$mail'";
